...
public class SessionController {
     ...
     * 检查所连路由器是否是lute
     */
    private ICheckRouterTypeListener mICheckRouterTypeListener;
    /**
     * Instance
     */
    ...
    public void setCheckRouterTypeListener(ICheckRouterTypeListener mICheckRouterTypeListener) {
        this.mICheckRouterTypeListener = mICheckRouterTypeListener;
    }
     ...
     */
    private SessionController(SessionUserInfo sessionUserInfo,boolean isCheckRouterType) {
        mSessionUserInfo = sessionUserInfo;
        connect(isCheckRouterType);
    }
     ...
     */
    public void connect(boolean isCheckRouterType) {
        if (mSession == null) {
            //ToastUtil.make("mSession == null");
            mThread = new Thread(new SshRunnable(isCheckRouterType));
            mThread.start();
            //ToastUtil.make("start fir");
            //执行到这了
        } else if (!mSession.isConnected()) {
            //ToastUtil.make("!mSession.isConnected()");
            mThread = new Thread(new SshRunnable(isCheckRouterType));
            mThread.start();
        }
        //ToastUtil.make("connect()执行完了");
    }
    ...
    public void setConnectionStatusListener(ConnectionStatusListener csl) {
        //ToastUtil.make("SessionController Listener执行了");
...
*/
    ...
    public boolean executeCommand(Handler handler, ExecTaskCallbackHandler callback, String command) {
        if (mSession == null || !mSession.isConnected()) {
            ...
            if (mShellController == null) {
                ...
                try {
                    mShellController.openShell(getSession(), handler);
                } catch (Exception e) {
    ...
    public class SshRunnable implements Runnable {
        private boolean isCheckRouterType = false;

        public SshRunnable(boolean isCheckRouterType) {
            this.isCheckRouterType = isCheckRouterType;
        }
        ...
        public void run() {
            ...
            //判断路由器类型时回调
            if (isCheckRouterType) {
                if (mICheckRouterTypeListener != null) {
                    if (mSession.isConnected()) {
                        mICheckRouterTypeListener.isLuteRouter();
                    } else {
                        mICheckRouterTypeListener.noLuteRouter();
                    }
                }
            }
